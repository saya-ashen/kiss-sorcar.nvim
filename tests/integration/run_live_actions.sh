...
if [ "${KISS_REAL_DAEMON_LIVE_ACTIONS_TEST:-}" != "1" ]; then
  echo "SKIP real daemon live-actions test; set KISS_REAL_DAEMON_LIVE_ACTIONS_TEST=1"
  exit 0
fi
...
mkdir -p "$repo/tmp"
fixture=$(mktemp -d "$repo/tmp/real-daemon-live-actions.XXXXXX")
...
env KISS_TEST_SOCKET="$socket_path" KISS_TEST_WORK_DIR="$work_dir" \
  ...
  nvim --headless -u NONE -l tests/integration/live_actions.lua
...
assert len(rows) == 2, f"expected two task_history rows, got {len(rows)}"
assert all(result and result != "Agent Failed Abruptly" for _, result in rows), rows
assert rows[-1][1] == "Task stopped by user", rows[-1][1]
PY

printf 'PASS isolated live actions persistence: empty tabs registry; 2 terminal task_history rows; workspace unchanged\n'
