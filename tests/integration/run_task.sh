...
if [ "${KISS_REAL_DAEMON_RUN_TEST:-}" != "1" ]; then
  echo "SKIP real daemon run test; set KISS_REAL_DAEMON_RUN_TEST=1"
  exit 0
fi
...
mkdir -p "$repo/tmp"
fixture=$(mktemp -d "$repo/tmp/real-daemon-run.XXXXXX")
...
env KISS_TEST_SOCKET="$socket_path" KISS_TEST_WORK_DIR="$work_dir" \
  ...
  nvim --headless -u NONE -l tests/integration/run_task.lua
...
with sqlite3.connect(home / "sorcar.db") as database:
    rows = database.execute("SELECT task, result FROM task_history").fetchall()
assert len(rows) == 1, f"expected one task_history row, got {len(rows)}"
assert rows[0][1] and rows[0][1] != "Agent Failed Abruptly", rows[0][1]
PY

printf 'PASS isolated run persistence: empty tabs registry; 1 completed task_history row; workspace unchanged\n'
